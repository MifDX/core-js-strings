...
function removeFirstOccurrences(str, value) {
  ...
  return int === -1 ? str : str.replace(value, '');
}
...
function removeLastOccurrences(str, value) {
  const lastInt = str.lastIndexOf(value);
  return lastInt === -1
    ? str
    : str.slice(0, lastInt) + str.slice(lastInt + value.length);
}
...
function findLongestWord(sentence) {
  return sentence
    .split(' ')
    .reduce((ac, el) => (el.length > ac.length ? el : ac), '');
}
...
function invertCase(str) {
  return str
    .split('')
    .map((itm) =>
      itm.toUpperCase() === itm ? itm.toLowerCase() : itm.toUpperCase()
    )
    .join('');
}
...
function getCardId(value) {
  ...
  return (
    cardDenomination.indexOf(value[value.length - 1]) * 13 +
    cardSuit.indexOf(value[0])
  );
}
